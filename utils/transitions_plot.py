...
ax = sns.heatmap(df_percent, cmap='crest', annot=df_all, fmt = '', linewidths=0.5, vmin=0, vmax=100)
...
# # Plot and annotate with percentages
# fig = plt.figure(figsize=(16, 8), constrained_layout=True)
# ax = sns.heatmap(df_percent, cmap='crest', annot=True, fmt=",.1f", linewidths=0.5)
# ax.xaxis.tick_top()
# ax.set_xticks(nlabels, labels, rotation=90)
# ax.set_yticks(nlabels, labels, rotation=0)

# plt.savefig(fig_heatmap[:-4] + "_percent.png", bbox_inches='tight', dpi=150)
# plt.close()
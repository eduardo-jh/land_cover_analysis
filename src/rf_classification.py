...
#     h5_features_season = h5py.File(fn_features_season, 'w')  # TODO: Uncomment to create dataset
...
#                 feat_arr += h5_features[feat_name][:]
#         # Average & save
#         feat_arr = np.round(np.round(feat_arr).astype(np.int16) / np.int16(len(season_feats[key]))).astype(np.int16)  # TODO: Uncomment to create dataset
#         h5_features_season.create_dataset(key, feat_arr.shape, data=feat_arr)  # TODO: Uncomment to create dataset
#     # Add PHEN features directly, no aggregation by season
#     for feat_name in feat_names:
#         if feat_name[:4] == 'PHEN':
#             print(f" **{feat_num:>4}: {feat_name:>15}")
#             # Extract data & save
#             feat_arr = h5_features[feat_name][:]  # TODO: Uncomment to create dataset
